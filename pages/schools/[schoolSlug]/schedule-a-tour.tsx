...
export async function getServerSideProps(context) {
  ...
  let customSeo = {
    fullHead: schoolData.seo.fullHead
      .replaceAll(`${schoolSlug}`, `${schoolSlug}/schedule-a-tour`)
      .replaceAll(`<title>`, `<title>Contact Us | `),
  };
  ...
  return {
    props: {
      ...
      schoolHours:
        "M-F " +
          schoolSettings?.hoursOfOperation.openingTime +
          " - " +
          schoolSettings?.hoursOfOperation.closingTime || "",
      schedulerEvent: schoolSettings?.schedulerEventsOffered || "",
      calendlyURLs: {
        inPersonTour:
          schoolData?.schoolCorporateSettings.calendlyEmbedUrls
            .calendlyInPersonTour || "",
        virtualTour:
          schoolData?.schoolCorporateSettings.calendlyEmbedUrls
            .calendlyVirtualTour || "",
        introductionPhoneCall:
          schoolData?.schoolCorporateSettings.calendlyEmbedUrls
            .calendlyIntroductionPhoneCall || "",
      },
      hiddenFields: {
...
export default function ScheduleATourPage({
  ...
  const metaTitle =
    corporate?.scheduleATourMeta?.title ??
    `Contact us | Primrose School of ${schoolTitle}`;
  ...
  const nonCalendlyDesc =
    "We’d love for your family to meet ours. Please fill out the form below and we’ll contact you about a tour.";
  const calendlyDesc =
    "We’d love for your family to meet ours. Please fill out the form below and select your tour date and time.";
  const formDescription =
    corporate.usesCalendly == true ? calendlyDesc : nonCalendlyDesc;
  ...
  useEffect(() => {
    let calendlySelected = document.getElementById(
      "choice_13_13_Yes",
    ) as HTMLInputElement;
  ...
  const handleCalendlySelect = (value) => {
    ...
    let prefillParams =
      "?embed_domain=www.primroseschools.com&embed_type=Inline&hide_gdpr_banner=1&name=" +
      calendlyName +
      "&email=" +
      calendlyEmail;
    ...
    if (value == "In-Person Tour") {
      ...
      setCalendlyEvent(
        calendlyURLs.introductionPhoneCall +
          prefillParams +
          "&location=1" +
          calendlyPhone,
      );
          ...
          <div className="main-wrapper col-12 col-lg-8">
            <div className="form-wrapper">
              <div className="heading-wrapper">
                <h2 id="sat-heading" className="heading green">
                  Schedule A Tour
                </h2>
                <p className="desc b3">{formDescription}</p>
              </div>
              <CalendlyContext.Provider
                value={{ isCalendlySelected, setCalendlySelected }}
              >
                <ScheduleATourForm {...hiddenFields} />
              </CalendlyContext.Provider>
              {corporate.usesCalendly &&
                (calendlyURLs.inPersonTour != "" ||
                  calendlyURLs.virtualTour != "" ||
                  calendlyURLs.introductionPhoneCall != "") &&
                schedulerEvent != "" && (
                  <div
                    id={"SAT-Calendly-Div"}
                    className="calendly-widget hidden"
                  >
                    <DynamicRadioButtons
                      skipURL={tourThanksSlug}
                      options={schedulerEvent}
                      onSelect={handleCalendlySelect}
                    />
                    <CalendlyEmbed
                      url={calendlyEvent}
                      successUrl={tourThanksSlug}
                    />
                  </div>
                )}
            </div>
          ...
          <div className="aside col-lg-3 offset-lg-1 d-none d-lg-flex flex-column">
            {corporate && corporate.address && (
              <div className="address b3 border-bottom pt-3 pb-3">
                ...
                {corporate.address.streetAddress2 && (
                  <>
                    <br />
                    {corporate.address.streetAddress2}
                  </>
            ...
            {schoolHours && (
              <div className="hours border-bottom pt-3 pb-3">
                <div
                  className="b3"
                  dangerouslySetInnerHTML={{ __html: schoolHours }}
                ></div>
              </div>
            )}
            ...
            <div className="social">
              <div className="h5 green">Connect with Us</div>
