...
export default function SchoolNewsItem() {
	...
	const readingTime = newsItem.content ? calculateReadingTime(newsItem.content.replace(/(<([^>]+)>)/ig, '')) : '';
		...
		<>
			<div className='resource'>
				...
				<div className='social d-flex justify-content-center flex-xl-column mt-4 mb-4 mt-xl-0'>
					<Link href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(usePathname())}`} target="_blank" rel="noopener noreferrer">
					...
					<Link href={`https://www.linkedin.com/shareArticle?mini=true&url=${encodeURIComponent(usePathname())}&title=${encodeURIComponent(newsItem.title)}&summary=${encodeURIComponent(newsItem.shortDescription?.substring(0, 150) || '')}&source=PrimroseSchools`} target="_blank" rel="noopener noreferrer">
