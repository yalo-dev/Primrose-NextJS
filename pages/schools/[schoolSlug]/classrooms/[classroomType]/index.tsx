...
export async function getServerSideProps(context) {
  ...
  try {
    ...
    let customSeo = {
      fullHead: data.classroom.seo.fullHead.replaceAll(
        `/classroom/${classroomType}`,
        `/schools/${schoolSlug}/classrooms/${classroomType}`,
      ),
    };
...
export default function ClassroomTypePage({ school, schoolSlug, data }) {
  ...
  const featuredBanner =
    school?.schoolSettings?.classrooms?.classroomSelection?.classroomDetails
      ?.infant?.featuredBanner || {};
  ...
  const shouldRenderCTA =
    hasData(featuredBanner) &&
    featuredBanner.icon &&
    featuredBanner.heading &&
    featuredBanner.blurb &&
    featuredBanner.button;
  const schoolAdminSettings =
    school?.schoolAdminSettings &&
    school?.schoolAdminSettings[camelize(classroomType) as string];
      ...
      if (tab.label == "Healthy Bodies") {
        tab.content.list.map((listitem) => {
          if (listitem.text == "Health & Safety") {
            ...
            listitem.detailsPopUp +=
              school?.schoolAdminSettings[
                camelize(classroomType) as string
              ]?.healthSafety;
  ...
  const testimonialSection = () => {
    ...
    const transformedTestimonials = adminSettings[
      camelize(classroomType) as string
    ]?.assignedTestimonials?.map((testimonial) => ({
      avatar: {
    ...
    if (classroomType.indexOf("school") != -1 && bussingInformation) {
      const bussingProps = {
        ...
        leftColumn: {
          imageDesktop: {
            sourceUrl:
              "https://settings.primroseschools.com/wp-content/uploads/2024/02/Bussing_LR.jpg",
  ...
  const StaffMembers = () => {
    ...
    const assignedStaffMembers = staffMembersData.filter(
      (member) =>
        member.classroomAssignment?.replace(" ", "-").toLowerCase() ===
        currentClassroomType,
    );
    const canLoadMore = assignedStaffMembers.length > visibleStaffCount;
    const classroomTypeString = Array.isArray(classroomType)
      ? classroomType[0]
      : classroomType;
    const classroomTypeFormatted = classroomTypeString
      ? classroomTypeString.charAt(0).toUpperCase() +
        classroomTypeString.slice(1).toLowerCase()
      : "Classroom";
                      ...
                      <div className="col-4">
                        {member.image ? (
                          ...
                          <Image
                            width={1920}
                            height={1920}
                            src={"/assets/staff-default-thumbnail.jpg"}
  ...
  const selectedDomains =
    school?.schoolSettings?.classrooms?.classroomSelection?.classroomDetails
      ?.infant?.learningDomains?.selectDomains || [];
  ...
  const heroProps = {
    ...
    rightColumn: {
      ...
      button: {
        ...
        title: school?.schoolCorporateSettings?.usesCalendly
          ? "Schedule A Tour"
          : "Contact Us",
  ...
  const metadataDefaultsKey =
    classroomType && typeof classroomType === "string"
      ? classroomType
      : classroomType[0];

  return (
    <div className="school classroom-type">
      {metadataDefaults[metadataDefaultsKey] && (
        <Head>
          <title>{metadataDefaults[metadataDefaultsKey].title}</title>
          <meta
            name={"description"}
            content={`${metadataDefaults[metadataDefaultsKey].description}`}
          />
        </Head>
      )}
      ...
      <div className="container">
        <h2 className="heading">
          {classroom?.classroomModules?.verticalTabsHeadline ??
