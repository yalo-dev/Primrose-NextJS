...
const JobPostPage = () => {
  ...
  const [jobDetails, setJobDetails] = useState<JobDetail | undefined>(
    undefined,
  );
  const [cpJobDetails, setCpJobDetails] = useState<JobDetail | undefined>(
    undefined,
  );
  ...
  const [cmsJob, setCmsJob] = useState<CMSJobDetail | null>(null); // Updated state with type
  ...
  const { data: cmsData, loading: cmsLoading } = useQuery(GET_SCHOOL_DETAILS, {
    variables: { id: schoolSlug },
    skip: !schoolSlug,
  });

  useEffect(() => {
    if (cmsData?.school?.schoolCorporateSettings?.careerplugSchoolId && jobId) {
      // Fetch CP jobs
      fetch(`/api/fetchJobs?jobId=${jobId}`)
        .then((response) => response.json())
        .then((data) => {
          setCpJobDetails(data);
        })
        .catch((error) =>
          console.error("Error fetching CP job details:", error),
        );
    } else if (
      !cmsData?.school?.schoolCorporateSettings?.careerplugSchoolId &&
      jobId
    ) {
      const job = cmsData?.school?.schoolAdminSettings?.jobPostings.find(
        (job) => job.slug === jobId,
      );
      setCmsJob(job);
    }
      ...
      <div className="container d-lg-flex justify-content-between">
        <div className="main">
          <div className="back-container mt-3 mb-4">
            <Link href={`/schools/${cmsData?.school?.slug}/careers`} passHref>
              <span className="arrow">
                ...
                >
                  <path
                    d="M5.78906 1L-0.000412507 6.26316L5.78906 11"
                    stroke="#18211F"
                  />
                  <path
                    d="M0.523438 6.26279L14.8655 6.26279"
                    stroke="#18211F"
                  />
                </svg>
              </span>{" "}
          ...
          {cpJobDetails ? (
            <>
              <h1>{cpJobDetails.name}</h1>
              <div className="location d-lg-flex align-items-center">
                <p className="b4 mb-0">{cpJobDetails.location.name}</p>
                <span className="d-none d-lg-flex">
                  &nbsp;&nbsp;|&nbsp;&nbsp;
                </span>
                <p className="b4 mb-0">
                  {cpJobDetails.city}, {cpJobDetails.state}
                </p>
              </div>
              <p className="employment-type mt-2 mb-2">
                {cpJobDetails.employment?.name}
              </p>
              <div
                dangerouslySetInnerHTML={{ __html: cpJobDetails.description }}
              ></div>
            </>
          ) : (
            cmsJob && (
              <>
                ...
                <div className="location d-lg-flex align-items-center">
                  <p className="b4 mb-0">
                    {cmsData.school.title || "No School Name"}
                  </p>
                  <span className="d-none d-lg-flex">
                    &nbsp;&nbsp;|&nbsp;&nbsp;
                  </span>
                  <p className="b4 mb-0">{`${cmsData.school.schoolCorporateSettings.address.city || "No City"}, ${cmsData.school.schoolCorporateSettings.address.state || "No State"}`}</p>
                </div>
                <p className="employment-type mt-2 mb-2">
                  {cmsJob.jobType || "No Type"}
                </p>
                <div
                  dangerouslySetInnerHTML={{ __html: cmsJob.jobDescription }}
                ></div>
              </>
            )
          )}
          <div className="social-sharing pt-5 d-none d-lg-block">
            ...
            <div className=" d-flex">
              ...
              <div className="twitter">
                <a
                  href={`https://twitter.com/intent/tweet?url=${encodeURIComponent(pageUrl)}&text=Check out this job opportunity!`}
                  target="_blank"
                  rel="noopener noreferrer"
                >
                  <svg
                  ...
                  >
                    <path
                      d="M23.2969 22.67L34.2635 37.33H36.7469L25.7802 22.67H23.2969Z"
                      fill="black"
                    />
                    <path
                      d="M30 15C21.7157 15 15 21.7157 15 30C15 38.2843 21.7157 45 30 45C38.2843 45 45 38.2843 45 30C45 21.7157 38.2843 15 30 15ZM33.6773 38.5013L28.6847 31.8257L22.5053 38.5013H20.917L27.975 30.8767L20.9613 21.4987H26.367L30.965 27.6467L36.6557 21.4987H38.244L31.6747 28.596L39.083 38.5013H33.6773Z"
                      fill="black"
                    />
                  </svg>
                </a>
              ...
              <div className="email">
                <a
                  href={`mailto:?subject=Interesting Job Opportunity&body=${encodeURIComponent(pageUrl)}`}
                  target="_blank"
                  rel="noopener noreferrer"
                >
                  <svg
                  ...
                  >
                    <path
                      d="M49.5 18.75C49.5 16.6875 47.8125 15 45.75 15H15.75C13.6875 15 12 16.6875 12 18.75V41.25C12 43.3125 13.6875 45 15.75 45H45.75C47.8125 45 49.5 43.3125 49.5 41.25V18.75ZM45.75 18.75L30.75 28.125L15.75 18.75H45.75ZM45.75 41.25H15.75V22.5L30.75 31.875L45.75 22.5V41.25Z"
                      fill="black"
                    />
                  </svg>
                </a>
              ...
              {isSubmitted ? (
                <div className="thank-you">
                  <div className="text-center">
                    <img
                      src="/assets/thankyouimage.png"
                      alt="thank you image"
                    />
                  </div>
                  <h4 className="mt-4">Thank you for your application!</h4>
                  <p className="b3">
                    We received your application and will be in touch soon. If
                    you subscribed to text messages, look out for a message from
                    us. We will also reach out via email.
                  </p>
                </div>
              ) : (
                <form
                ...
                >
                  ...
                  {currentStep === 1 && (
                    <div className="first-page" id="step1">
                      ...
                      <div className="checkbox-wrap d-flex align-items-start">
                        ...
                        <label htmlFor="optInText">
                          Yes, text me! For the fastest response to interviews &
                          follow-ups. Opt-out anytime.
                        </label>
                      </div>
                  ...
                  {currentStep === 2 && (
                    <div className="second-page" id="step2">
                      ...
                      <label htmlFor="schedule">
                        This position may require you to work certain evenings
                        (for marketing/events) and/or Saturdays. Can you work
                        this schedule?*
                      </label>
                      <SelectDropdown
                      ...
                      <label htmlFor="liftUp">
                        Are you able to lift up to 35 lbs?*
                      </label>
                      <SelectDropdown
                      ...
                      <label htmlFor="references">
                        Are you willing to provide us with 3 professional
                        references?*
                      </label>
                      <SelectDropdown
                  ...
                  <div className="pagination d-flex justify-content-center align-items-center pt-4 pb-4">
                    <span
                      className={`arrow-left me-2 ${currentStep === 1 ? "disabled" : ""}`}
                    ...
                    >
                      ...
                      >
                        <ellipse
                          ...
                          rx="15"
                          ry="15"
                          transform="matrix(1.31134e-07 -1 -1 -1.31134e-07 30.5 30)"
                          fill="#FBFBFB"
                        />
                        <path
                          fillRule="evenodd"
                          clipRule="evenodd"
                          d="M17.1765 20.794C17.5579 20.4797 17.6095 19.9192 17.2916 19.542L13.2979 14.8032L17.2662 10.4868C17.6002 10.1236 17.5731 9.5614 17.2057 9.23117C16.8384 8.90094 16.2698 8.92771 15.9359 9.29096L11.9676 13.6073C11.364 14.2637 11.3423 15.2597 11.9167 15.9413L15.9105 20.6801C16.2283 21.0573 16.7951 21.1082 17.1765 20.794Z"
                          fill="#555F68"
                        />
                      </svg>
                    </span>
                    ...
                    <span
                      className={`arrow-right ms-2 ${currentStep === 2 ? "disabled" : ""}`}
                    ...
                    >
                      ...
                      >
                        ...
                        <path
                          fillRule="evenodd"
                          clipRule="evenodd"
                          d="M13.3235 9.20605C12.9421 9.52033 12.8905 10.0808 13.2084 10.458L17.2021 15.1968L13.2338 19.5132C12.8998 19.8764 12.9269 20.4386 13.2943 20.7688C13.6616 21.0991 14.2302 21.0723 14.5641 20.709L18.5324 16.3927C19.136 15.7363 19.1577 14.7403 18.5833 14.0587L14.5895 9.31986C14.2717 8.94272 13.7049 8.89177 13.3235 9.20605Z"
                          fill="#555F68"
                        />
                      </svg>
                    </span>
