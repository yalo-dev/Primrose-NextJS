...
export async function getServerSideProps(context) {
  ...
  try {
    ...
    const careerPlugSchoolId =
      school.schoolCorporateSettings.careerplugSchoolId || null;
    let customSeo = {
      fullHead: school.seo.fullHead
        .replaceAll(`${schoolSlug}`, `${schoolSlug}/careers`)
        .replaceAll(`<title>`, `<title>Careers | `),
    };
...
export default function SchoolCareerPage({ school }) {
  ...
  const metaTitle =
    school.schoolCorporateSettings?.careersMeta?.title ??
    `Careers | Primrose School of ${school?.title}`;
  ...
  const metaDesc =
    school.schoolCorporateSettings?.careersMeta?.description ?? defaultDesc;
  const { usesCareerplug, careerplugIframeUrl } =
  ...
  return (
    <div className="school school-careers">
            ...
            {school.schoolAdminSettings?.careersIntroCopy ? (
              <div
                ...
                dangerouslySetInnerHTML={{
                  __html: school.schoolAdminSettings?.careersIntroCopy,
                }}
              />
            ) : (
              <p className="b3">
                We're growing. And we're looking for dedicated individuals who
                are as excited about helping children develop and learn as we
                are. If you're passionate about education and nurturing children
                and are looking for an environment with high standards for
                health and safety, consider a career with us.
              </p>
            )}
          </div>
          ...
          {usesCareerplug ? (
            careerplugIframeUrl && (
              <section className="careerplug-section">
                ...
                />
                <Script
                  src={"https://cpats.s3.amazonaws.com/assets/embed.js"}
                />
              </section>
            )
          ) : (
              ...
              {cmsJobs.length > 0 ? (
                cmsJobs.map((job, index) => (
                  <div key={index} className="job-tile">
                    ...
                    <p className="b3 green mb-2">
                      {school.title ?? "No School Name"}
                    </p>
                    <p className="b2">{`${city ?? "No City"}, ${state ?? "No State"}`}</p>
                    <p className="employment-type mb-3">
                      {job.jobType ?? "No Type"}
                    </p>
                    <p className="b2 post-date">
                      Posted: {job.postDate ?? "No Date"}
                    </p>
                    <Button
                      variant="primary"
                      href={`/schools/${school.slug}/careers/${job.slug}`}
                    >
                      Learn More
                    </Button>
                  </div>
      ...
      {sliderItems && (
        <section
          className="module Page_Modules_Modules_TestimonialsWithVideoOrImage"
      ...
      {gallery?.length && (
        <GallerySlider gallery={gallery} uniqueId="gallerySlider" />
      )}
      {/* End Gallery Section */}
    </div>
  );
