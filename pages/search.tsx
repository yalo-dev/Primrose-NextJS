...
const SearchPage = ({ schoolsOverview }) => {
    ...
    const fetchResourceTags = async () => {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_WORDPRESS_API_URL}/wp-json/wp/v2/resource_tag?per_page=100`,
        );
  ...
  const fetchNames = async (
    ...
    for (const id of ids) {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_WORDPRESS_API_URL}/wp-json/wp/v2/${endpoint}/${id}?per_page=10&page=1`,
        );
  ...
  const fetchSearchResults = async (searchTerm: string) => {
    ...
    try {
      ...
      const batchResults = await Promise.all(
        baseUrls.map((url) => fetchBatch(url)),
      );
      const flatResults = batchResults.flat();

      const resultsWithAdditionalData = await Promise.all(
        flatResults
          .filter((post: SearchResult) => !post.url.includes("-st"))
          .map(async (resource) => {
            const enhancedResource: SearchResult = { ...resource };

            if (resource.featured_media) {
              const mediaResponse = await fetch(
                `${process.env.NEXT_PUBLIC_WORDPRESS_API_URL}/wp-json/wp/v2/media/${resource.featured_media}`,
              );
            ...
            if (resource.resource_type) {
              enhancedResource.resourceTypeNames = await fetchNames(
            ...
            if (resource.resource_tag) {
              enhancedResource.resourceTagNames = await fetchNames(
      ...
      const newsExcludedResults = resultsWithAdditionalData.filter(
        (post: SearchResult) => !post.url.includes("/news/"),
      );
  ...
  const handleTagSelection = (selectedValues: string[]) => {
    ...
    resourceCards.forEach((card) => {
      ...
      const matchesFilter = selectedClasses.some(
        (className) => className && htmlCard.classList.contains(className),
      );
    ...
    const renderTitleAndFourPanels = () => (
      <div className="container col-lg-10 offset-lg-1">
        {!titleLoading && !titleError && (
          <div className="b4 pt-4">
            {titleData.siteSettings.siteSettings.titleFor4Panels}
          </div>
        )}
        <FourPanels />
      </div>
              ...
              <div className="container col-lg-10 offset-lg-1">
                {paginatedTopResults.map((post) => (
                  <div className="result" key={post.id}>
                    ...
                    <a className="b2 link" href={post?.url}>
                      {post?.url}
                    </a>
                  </div>
                ))}
    ...
    <div className="search-container">
              ...
              >
                ...
                >
                  ...
                  <rect
                    ...
                    width="1.28571"
                    height="9"
                    transform="rotate(45 13.2266 7.71297)"
                    fill="#5E6738"
                  />
                  <rect
                    x="6.86719"
                    ...
                    width="1.28571"
                    height="9"
                    transform="rotate(-45 6.86719 8.62239)"
                    fill="#5E6738"
                  />
                </svg>
              </div>
      ...
      <div className="results">
        ...
        <Pagination
          ...
          itemCount={getTotalFilteredResults()}
      ...
      <div className="container col-lg-10 offset-lg-1">
        {searchPerformed &&
          ...
          getFilteredResults().length > 0 && <FourPanels />}
      </div>
    </div>
  );
};
