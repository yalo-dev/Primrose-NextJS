...
export default function RadioField({
  ...
  const introductionCallOnly =
    "Would you like to schedule your introduction call now?";
  const inPersonOrVirtualTourOnly = "Would you like to schedule your tour now?";
  const introductionCallAndTours =
    "Would you like to schedule your introduction call or tour now?";
  ...
  if (
    ...
    (hiddenFields.calendlyURLs.inPersonTour != "" ||
      hiddenFields.calendlyURLs.virtualTour != "" ||
      hiddenFields.calendlyURLs.introductionPhoneCall != "") &&
    hiddenFields.hasCalendlyEvent != ""
  ) {
    let schedulerLabel = "";
    let hasIntroPhoneCall = hiddenFields.hasCalendlyEvent.includes(
      "Introduction Phone Call",
    );
    let hasVirtualTour = hiddenFields.hasCalendlyEvent.includes("Virtual Tour");
    let hasInPersonTour =
      hiddenFields.hasCalendlyEvent.includes("In-Person Tour");
    if (
      hasIntroPhoneCall == false &&
      (hasVirtualTour == true || hasInPersonTour == true)
    ) {
      schedulerLabel = inPersonOrVirtualTourOnly;
    } else if (
      hasIntroPhoneCall == true &&
          ...
          {choices?.map((input) => {
            ...
            return (
              <div className="input-wrapper" key={inputValue}>
                ...
                <label htmlFor={`choice_${databaseId}_${id}_${inputValue}`}>
                  {text}
                </label>
              </div>
            );
