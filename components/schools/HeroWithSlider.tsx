...
export default function HeroWithSlider({
  ...
  const defaultImages = [
    {
      url: "/schoolsHomeDefault/header-default-1.jpg",
      altText: "A girl taking notes in class",
    },
    {
      url: "/schoolsHomeDefault/header-default-2.jpg",
      altText: "A boy playing with giant lego blocks",
    },
    {
      url: "/schoolsHomeDefault/header-default-3.jpg",
      altText: "A boy looking into the camera",
    },
  ];
  const sliderImages = corporateSettings.homepageHeroImage
    ? [
        ...corporateSettings.homepageHeroImage.map((image) => ({
              ...
              <div className="alert preopening-alert">
                <div className="row align-items-center">
                  <div className="col-12 d-flex justify-content-start align-items-center">
                    ...
                    <h5 className="alert-title mb-0">
                      We are Opening in {corporateSettings?.openingIn?.season}{" "}
                      {corporateSettings?.openingIn?.year}
                    </h5>
                  </div>
                </div>
              </div>
                  ...
                  {sliderImages.map((image, index) => (
                    <div className="image-wrapper d-block" key={index}>
                      <Image
                        ...
                        src={image.url}
                        alt={image.altText || `Hero Image ${index}`}
                      />
                    </div>
                  ))}
                </Slider>
              </div>
              {corporateSettings?.virtualTourUrl && (
                <Link
                  className={"google-tour-link"}
                  href={corporateSettings?.virtualTourUrl}
                  target={"_blank"}
                >
            ...
            <div className="col right-col col-12 col-lg-6">
              {adminSettings?.hiringNow && (
                <div className="alert hiring-alert">
                  <div className="row align-items-center">
                    <div className="col">
                      <h5 className="alert-title text-white mb-0">
                        Now Hiring
                      </h5>
                    </div>
                    <div className="col text-end">
                      <a
                        href={`/schools/${schoolSlug}/careers`}
              ...
              <div className="d-lg-flex">
                <div className="info-wrapper">
                  ...
                  <div className="classrooms">
                    ...
                    <ul>
                      {sortedSelectedOfferings &&
                        sortedSelectedOfferings
                          .filter(
                            (classroom) => classroom !== "Before & After Care",
                          )
                          .map((classroom, index) => {
                  ...
                  <div className="hours mt-1">
                    <h5 className="green">Hours & Location</h5>
                    <span className="b3">
                      M-F {adminSettings?.hoursOfOperation.openingTime}-
                      {adminSettings?.hoursOfOperation.closingTime}
                    </span>
                  </div>
                  <div className="phone">
                    <span className="b3">
                      <a href={`tel:${corporateSettings?.phoneNumber}`}>
                        {corporateSettings?.phoneNumber}
                      </a>
                    </span>
                  </div>
                  <div className="address">
                    ...
                    <span className="b3">
                      {corporateSettings?.address?.streetAddress}
                      {corporateSettings?.address?.streetAddress2 &&
                        `, ${corporateSettings.address.streetAddress2}`}
                      , {corporateSettings?.address?.city},{" "}
                      {corporateSettings?.address?.state}{" "}
                      {corporateSettings?.address?.zipcode}
                    </span>
                  </div>
                </div>
                {(corporateSettings?.accreditations ||
                  ...
                  <div className="accreditations">
                    ...
                    <div className="accreditation-images  d-flex">
                      {corporateSettings.accreditations?.map(
                        (accreditation, index) =>
                          accreditation && (
                            <div
                            ...
                            >
                              <img
                                className="me-2 me-lg-0 mb-lg-2"
                                width="60"
                                height="60"
                                src={
                                  accreditation.accreditations.image
                                    .mediaItemUrl
                                }
                                alt={
                                  accreditation.title || "Accreditation Image"
                                }
                              />
                            </div>
                          ),
              ...
              <div className="social-links d-flex justify-content-center align-center border-top border-bottom mt-3 mb-3 mb-lg-0 pt-2 pb-2">
                ...
                {adminSettings?.instagramLink && (
                  <a
                    href={adminSettings.instagramLink}
                    target="_blank"
                    title="Instagram"
                    rel="noopener noreferrer"
                  >
