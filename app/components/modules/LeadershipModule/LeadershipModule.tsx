...
import SelectDropdown, {
  OptionType,
} from "../../molecules/SelectDropdown/SelectDropdown";
...
export default function LeadershipPage({
  leadershipMembers,
}: LeadershipMemberListProps) {
  const [activeBio, setActiveBio] = useState(null);
  const [bioHeights, setBioHeights] = useState({});
  const initialLeadershipCount = 20;
  const [visibleLeadershipCount, setVisibleLeadershipCount] = useState(
    initialLeadershipCount,
  );
  const [filteredLeadershipMembers, setFilteredLeadershipMembers] =
    useState<LeadershipMember[]>(leadershipMembers);
  ...
  const canLoadMore =
    filteredLeadershipMembers?.length > visibleLeadershipCount;
    ...
    const filtered =
      !selectedGroup || selectedGroup.value === "All"
        ...
        : leadershipMembers.filter(
            (member) => member.group === selectedGroup.value,
          );
      ...
      <div className="staff">
        <div className="staff-members-section">
          ...
          <div className="staff-members">
            {filteredLeadershipMembers
              ?.slice(0, visibleLeadershipCount)
              .map((member, index) => (
                <div
                    ...
                    <div className="col-4">
                      {member.image ? (
                        ...
                        <img
                          ...
                          alt="Primrose Leadership Member Photo"
                          className="img-fluid"
                        />
                      )}
                    </div>
          ...
          <div className="load-more d-flex align-items-center justify-content-center">
            {canLoadMore && (
              <Button onClick={loadMoreLeadership}>Load More</Button>
            )}
          </div>
        </div>
      </div>
