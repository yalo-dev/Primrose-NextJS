...
const Q1Skills: React.FC<Q1SkillsProps> = ({
  eyebrow,
  eyebrowColor,
  heading,
  headingColor,
  ...
  useEffect(() => {
    function handleOutsideClick(event: MouseEvent) {
      if (
        ...
        activePopup !== null &&
        !buttonsRef.current[activePopup]?.contains(event.target as Node)
      ) {
        setActivePopup(null);
      }
    }
    ...
    <div className="container">
      ...
      >
        <div className="q1skills">
          {eyebrow && (
            <Subheading level="h5" color={eyebrowColor}>
              {eyebrow}
            </Subheading>
          )}
          {heading && (
            <Heading level="h3" color={headingColor}>
              {heading}
            </Heading>
          )}
          {subheading && (
            ...
            {list.map((item, idx) => (
              <li key={idx}>
                <div className="icon-and-popup-container d-flex flex-lg-column align-lg-center justify-content-lg-center">
                  ...
                  <div
                    className={`details-popup ${activePopup === idx ? "active" : ""}`}
                  >
                    <div className="title-container">
                      ...
                      {item.description && (
                        <Subheading level="div" className="desc">
                          {item.description}
                        </Subheading>
                      )}
                    </div>
                    {item.detailsPopUp && (
                      <div
                        ...
                        dangerouslySetInnerHTML={{ __html: item.detailsPopUp }}
                      ></div>
                    )}
                  </div>
