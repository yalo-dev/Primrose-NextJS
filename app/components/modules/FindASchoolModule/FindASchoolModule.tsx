...
const FindASchoolMap: React.FC<FindASchoolMapProps> = (props) => {
  let { schools, heading, center, cta } = props;
  //console.log(props);
  if (center.latitude && center.longitude) {
    center = { lat: center.latitude, lng: center.longitude };
  } else if (schools && schools.length > 0) {
    const totalSchools = schools.length;
    const totalLat = schools.reduce(
      (sum, school) => sum + school.schoolCorporateSettings.address.latitude,
      0,
    );
    const totalLng = schools.reduce(
      (sum, school) => sum + school.schoolCorporateSettings.address.longitude,
      0,
    );
  ...
  const [autocomplete2, setAutocomplete2] =
    useState<google.maps.places.Autocomplete | null>(null);
  const [autocomplete3, setAutocomplete3] =
    useState<google.maps.places.Autocomplete | null>(null);
  ...
  const [userLocation, setUserLocation] =
    useState<google.maps.LatLngLiteral | null>(center);
  let geocoder;
  ...
  const [waypointRefs, setWaypointRefs] = useState<
    Record<number, React.RefObject<HTMLInputElement>>
  >({});
  ...
  const handleAddMoreClick = () => {
    ...
    if (!isMobile) {
      ...
      if (container) {
        if (
          getComputedStyle(container).getPropertyValue("--view-height") ===
          "100%"
        ) {
  ...
  const handleClearIconClick = (idToRemove: number) => {
    setWaypoints((prevWaypoints) => {
      const updatedWaypoints = prevWaypoints.filter(
  ...
  const renderRoute = () => {
    ...
    const mappedWaypoints: google.maps.DirectionsWaypoint[] = waypoints
      ...
      .filter(
        (waypoint): waypoint is google.maps.DirectionsWaypoint =>
          waypoint !== undefined,
      );
  ...
  const onWaypointSelected = (waypointId, selectedPlace) => {
    if (
      ...
      const formattedPlaceName = selectedPlace.name
  ...
  const updateLocationData = (items) => {
    ...
    const newWaypoints = items
      .slice(1, -1)
      .map((item) => ({ id: item.id, location: item.location }));
  ...
  return (
    <div
      id="map"
      className={"find-a-school-container " + (heading ? "title" : "")}
    >
      {heading && (
      ...
      <div className="search-box-container">
        <div className="tabs">
          ...
          <div
            className={`tab-content tab-content-1 ${activeTab === 1 ? "active" : ""}`}
          >
              ...
              >
                ...
                >
                  <path
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M4.05063 4.20281C-0.167919 8.47353 -0.167919 15.4082 4.05063 19.6786L11.6936 27.4167L19.3365 19.6786C23.555 15.4082 23.555 8.47353 19.3365 4.20281C15.1185 -0.0676034 8.26862 -0.0676034 4.05063 4.20281ZM11.8376 16.5565C14.384 16.5565 16.4485 14.4539 16.4485 11.8602C16.4485 9.26653 14.384 7.16391 11.8376 7.16391C9.29132 7.16391 7.22679 9.26653 7.22679 11.8602C7.22679 14.4539 9.29132 16.5565 11.8376 16.5565Z"
                    stroke="#555F68"
                    strokeWidth="1.5"
                  />
                </svg>
              ...
              >
                ...
                >
                  <circle
                    ...
                    cy="14.8492"
                    r="9.75"
                    transform="rotate(45 14.8516 14.8492)"
                    stroke="#5E6738"
                    strokeWidth="1.5"
                  />
                  <rect
                    ...
                    width="1.28571"
                    height="9"
                    transform="rotate(45 17.5781 11.2129)"
                    fill="#5E6738"
                  />
                  <rect
                    x="11.2188"
                    ...
                    width="1.28571"
                    height="9"
                    transform="rotate(-45 11.2188 12.1211)"
                    fill="#5E6738"
                  />
                </svg>
              </div>
          ...
          <div
            className={`tab-content tab-content-2 ${activeTab === 2 ? "active" : ""}`}
          >
                ...
                <div className="add">
                  ...
                  >
                    ...
                    <path
                      fillRule="evenodd"
                      clipRule="evenodd"
                      d="M12.5 24.0001C18.299 24.0001 23 19.2991 23 13.5001C23 7.70113 18.299 3.00012 12.5 3.00012C6.70101 3.00012 2 7.70113 2 13.5001C2 19.2991 6.70101 24.0001 12.5 24.0001ZM11.8594 9.00012H13.1451V12.857H17V14.1427H13.1451V18.0001H11.8594V14.1427H8V12.857H11.8594V9.00012Z"
                      fill="#FF9E1B"
                    />
                  </svg>
                </div>
                ...
                <Droppable droppableId="routeInputs">
                  {(provided) => (
                    <div
                      ...
                      className={`input-wrapper ${isAdded ? "added" : ""}`}
                    >
                      ...
                      {waypoints.map((waypoint, index) => {
                        const waypointInputField = inputFields.find(
                          (field) => field.id === `waypoint-${waypoint.id}`,
                        );
                        ...
                        return (
                          <div key={waypoint.id} className="waypoint-input">
                            ...
                            <Draggable
                              key={waypoint.id}
                              draggableId={`waypoint-${waypoint.id}`}
                              index={index + 1}
                            >
                              {(provided) => (
                                <div
                            ...
                            <div
                              className="clear-icon"
                              onClick={() => handleClearIconClick(waypoint.id)}
                            >
                              <svg
                                width="30"
                                height="30"
                ...
                <a href={`${school.uri}`}>
                  ...
                  >
                    ...
                    <div className="wrap">
                      ...
                      <a
                        href={
                          school.schoolCorporateSettings.address.googlePlaceUrl
                        }
                        className="address"
                      >
                    ...
                    <div className="hours">
                      {"M-F " +
                        school.schoolAdminSettings?.hoursOfOperation
                          ?.openingTime +
                        " - " +
                        school.schoolAdminSettings?.hoursOfOperation
                          ?.closingTime}
                    ...
                    <div className="button-wrap d-flex">
                      <Button
                        ...
                        href={
                          cta?.href
                            ? `/schools/${school.slug}/careers`
                            : `/schools/${school.slug}/schedule-a-tour`
                        }
                      >
                        {cta?.title || "Schedule a Tour"}
                      </Button>
                      <a
                        href={`tel:${school.schoolCorporateSettings.phoneNumber}`}
                        className="phone ms-2"
                      >
                        <svg
                          width="50"
                          height="50"
      ...
      <div
        className={`google-map-container ${isMobile ? (!showMap || !hasSearched ? "hidden" : "") : hasSearched ? "shift" : ""}`}
      >
        <GoogleMap
          center={mapCenter}
          zoom={zoomLevel}
        ...
        >
          ...
          {waypoints &&
            waypoints
              .filter(
                (waypoint): waypoint is { id: number; location: Location } =>
            ...
            <div key={index} className="school-list">
              <a href={`${school.uri}`}>
                ...
                >
                  ...
                  <div className="hours">
                    {"M-F " +
                      school.schoolAdminSettings.hoursOfOperation?.openingTime +
                      " - " +
                      school.schoolAdminSettings.hoursOfOperation?.closingTime}
                  </div>
                  {/* <ul className='notes'><li>{school.notes}</li></ul> */}
                  <div className="button-wrap d-flex">
                    <Button
