  ...
  return (
    <div className="container">
      <Customizations {...customizations}>
        <div
          className={`pathway-to-ownership row ${isSticky ? "sticky" : ""}`}
          ref={pathwayRef}
        >
          <div className="col-12 col-lg-5 offset-lg-1">
            <div className="wrap">
              <Heading level="h2" className="green" color={headingColor}>
                {heading}
              </Heading>
            ...
            {boxes?.map((box, index) => (
              <div key={index} className="box">
                <div className="icon">
                  {box.icon?.sourceUrl && (
                    <img
                      src={box.icon.sourceUrl}
                      alt={box.icon.altText || ""}
                      width="25"
                      height="25"
                    />
                  )}
                </div>
                <Heading level="h3" color={box.headingColor}>
                  {box.heading}
                </Heading>
                <Subheading
                  level="div"
                  className="b3"
                  color={box.subheadingColor}
                >
                  {box.subheading}
                </Subheading>
                <ul>
                  {box.list &&
                    box.list.map((listItem, listItemIndex) => (
                      <li key={listItemIndex} className="b2">
                        <ColorComponent color={listItem.listItemColor}>
                          {listItem.listItem}
                        </ColorComponent>
                      </li>
                    ))}
