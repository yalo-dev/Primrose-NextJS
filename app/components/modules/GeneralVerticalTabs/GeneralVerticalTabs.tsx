      ...
      <div className="container" ref={containerRefVT}>
        <div className="general-vertical-tabs">
          ...
          <div className="heading-wrapper">
            ...
            {subheading && (
              <Subheading level="div" className="b4">
            ...
            {tabs?.map((tab, index) => (
              <div key={index} className="mobile-tab">
                ...
                >
                  <Heading level="h5">
                    {tab.label && (
                      <ColorComponent color={tab.tabLabelColor}>
                        {tab.label}
                      </ColorComponent>
                    )}
                  </Heading>
                ...
                >
                  {tab.content?.image?.sourceUrl && (
                    <div
                      className={`image-wrapper ${tab.content?.fullWidthOrFeatured}`}
                    >
                      <img
                  ...
                  <div className="content">
                    ...
                    {tab.content?.subheading && (
                      <Subheading level="div" className="subhead b4">
                    ...
                    {tab.content?.blurb && (
                      <div
                        className="blurb b3"
                        dangerouslySetInnerHTML={{ __html: tab.content?.blurb }}
                      />
                    )}
                      ...
                      <div className="table">
                        {tab.content?.table.map((item, idx) => (
                          <div className="d-flex pt-2 pb-2" key={idx}>
                            <div className="label h5 w-50">{item.label} </div>
                            <div className="desc b2 ms-4 w-50">
                              {item.description}
                            </div>
                          </div>
                        ))}
              ...
              {tabs?.map((tab, index) => (
                <div
                  ...
                  className={`tab-content ${
                    expandedTabVT === index ? "expanded fade-in" : "fade-out"
                  }`}
                >
                  <div className="content col-6">
                    ...
                    {tab.content?.table && (
                      <div className="table">
                        {tab.content?.table.map((item, idx) => (
                          <div className="d-flex pt-2 pb-2" key={idx}>
                            <div className="label h5 w-25">{item.label} </div>
                            <div className="desc b2 ms-4 w-75">
                              {item.description}
                            </div>
                          </div>
                        ))}
