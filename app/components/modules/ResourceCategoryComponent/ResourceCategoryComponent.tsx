...
export default function CategoryComponent() {
  ...
  useEffect(() => {
    if (data && slug) {
      ...
      const categorySpecificResources = allResources.filter(resource =>
        resource.resourceTypes.nodes.some(type => type.slug === slug)
      );

      const featured = categorySpecificResources.filter(resource =>
  ...
  return (
    <div className='container category'>
      <div className='resources-container'>
        {featuredResources.length > 0 ? renderResourceList(featuredResources, true, 'featured') : <p>No Featured Resources</p>}
      </div>
      ...
      <div id='all' className='resources-container'>
        ...
        {currentResources && currentResources.length > 0 ? renderResourceList(currentResources) : <p>No Resources Found</p>}
      </div>
      {Pagination()}
