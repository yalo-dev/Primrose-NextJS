...
const GallerySlider = ({ gallery, uniqueId }) => {
  ...
  const handleImageLoad = (index, width) => {
    const slideWidthWithMargin =
      index < gallery.length - 1 ? width + 16 : width;
  ...
  useEffect(() => {
    ...
    const imageElements = sliderContainer.querySelectorAll(
  ...
  useEffect(() => {
    ...
    setIsNextArrowDisabled(
      isLastSlideFullyVisible || currentIndex === gallery.length - 1,
    );
          ...
          {gallery?.map((item, index) => (
            ...
            >
              <Image
                ...
                height={1920}
                src={
                  item.image?.sourceUrl ?? "/assets/staff-default-thumbnail.jpg"
                }
