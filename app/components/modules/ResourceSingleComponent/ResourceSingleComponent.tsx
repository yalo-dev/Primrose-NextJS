...
export default function ResourceComponent({ singleSlug }) {
  ...
  const readingTime = content
    ? calculateReadingTime(content.replace(/(<([^>]+)>)/gi, ""))
    : "";
  ...
  const sortedTags = [...resource.resourceTags.nodes].sort(
    ...
    <>
      <div className="resource">
              ...
              <div className="content-wrapper ps-4 pe-4 position-relative">
                ...
                {displayAuthor || readingTime ? (
                  <div className="utils-wrapper pt-2 pb-4 d-flex align-items-center justify-content-between">
                    {displayAuthor && author?.node?.name && (
                      <div className="author">By: {author.node.name}</div>
                    )}
                    {readingTime && (
                      <div className="read-time">
                        <p className="b1 mb-0">{readingTime}</p>
                      </div>
                    )}
                  </div>
                ) : null}
        ...
        <div className="social d-flex justify-content-center flex-xl-column mt-4 mb-4 mt-xl-0">
          <Link
            href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(singleSlug)}`}
            target="_blank"
          ...
          <Link
            href={`https://twitter.com/intent/tweet?url=${encodeURIComponent(singleSlug)}&text=${encodeURIComponent(resource.title)}`}
            target="_blank"
            rel="noopener noreferrer"
          >
            <svg
          ...
          <Link
            href={`https://www.linkedin.com/shareArticle?mini=true&url=${encodeURIComponent(singleSlug)}&title=${encodeURIComponent(resource.title)}&summary=${encodeURIComponent(resource.resourceFields.content?.substring(0, 150) || "")}&source=PrimroseSchools`}
            target="_blank"
            rel="noopener noreferrer"
          >
            <svg
        ...
        {wrappedContent && (
          <div
            className="resource-content p-3 p-lg-0 mx-auto"
        ...
        {newsletterFormCta &&
          (newsletterFormCta.heading ||
            newsletterFormCta.subheading ||
            newsletterFormCta.accentOne ||
            newsletterFormCta.accentTwo) && (
            <div className="container mt-lg-4 pt-lg-4">
              <div className="newsletter-form-cta">
                <div className="row">
                  <div className="col-12 content">
                    {newsletterFormCta.heading && (
                      <Heading level="h3">{newsletterFormCta.heading}</Heading>
                    )}
                    {newsletterFormCta.subheading && (
                      <Paragraph className="b3">
                        {newsletterFormCta.subheading}
                      </Paragraph>
                    )}
                    <NewsletterForm />
                    <div
                      ...
                      style={{
                        backgroundImage: `url('${newsletterFormCta.accentOne?.sourceUrl || "defaultImageURL.jpg"}')`,
                      }}
                    ></div>
                    <div
                      className="accent-two"
                      style={{
                        backgroundImage: `url('${newsletterFormCta.accentTwo?.sourceUrl || "defaultImageURL.jpg"}')`,
                      }}
                    ></div>
                  </div>
                </div>
        ...
        {seasonalBanner &&
          (seasonalBanner.heading ||
            seasonalBanner.subheading ||
            seasonalBanner.button ||
            seasonalBanner.accentOne ||
            seasonalBanner.accentTwo ||
            seasonalBanner.accentThree) && (
              ...
              <div className="seasonal-banner">
                <div className="row">
                  ...
                  <div className="col-12 col-lg-6 bottom">
                    {seasonalBanner.heading && (
                      <Heading level="h3">{seasonalBanner.heading}</Heading>
                    )}
                    {seasonalBanner.subheading && (
                      <Paragraph className="b3">
                        {seasonalBanner.subheading}
                    ...
                    {seasonalBanner.button?.title &&
                      seasonalBanner.button.url && (
                        <Button
                          variant="primary"
                  ...
                  <div
                    className="accent-one"
                    style={{
                      backgroundImage: `url('${seasonalBanner.accentOne?.sourceUrl || "defaultImageURL.jpg"}')`,
                    }}
                  ></div>
                  <div
                    className="accent-two"
                    style={{
                      backgroundImage: `url('${seasonalBanner.accentTwo?.sourceUrl || "defaultImageURL.jpg"}')`,
                    }}
                  ></div>
                  <div
                    className="accent-three"
                    style={{
                      backgroundImage: `url('${seasonalBanner.accentThree?.sourceUrl || "defaultImageURL.jpg"}')`,
                    }}
                  ></div>
                </div>
              </div>
