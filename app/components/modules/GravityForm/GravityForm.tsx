    ...
    for (let i = 0; i < e.target.elements.length; i++) {
      ...
      if (element.type == "email") {
        fieldValues.push({
          ...
          emailValues: { value: element.value },
        });
...
const GFInput = (field) => {
  ...
  if (field.field.conditionalLogic) {
    classes += " hide";
    conditionals.push({
      trigger: field.field.conditionalLogic.rules[0].fieldId,
      field: field.field.id,
      operator: field.field.conditionalLogic.rules[0].operator,
      value: field.field.conditionalLogic.rules[0].value,
    });
  }
  ...
  switch (field.field.type) {
    case "SELECT":
      return (
        <div
          key={field.field.id}
          id={`gform_${field.field.id}`}
          className={classes}
        >
          <label
            className={field.field.isRequired ? "required" : ""}
    ...
    case "RADIO":
            ...
            {field.field.choices.map((choice, index) => (
              <div key={index}>
                <input
                  ...
                  onChange={handleFieldChange}
                  type="radio"
                  checked={field.field.isSelected}
                  name={field.field.id}
                  id={`${field.field.id}_${index}`}
                  value={choice.value}
                />
                <label htmlFor={`${field.field.id}_${index}`}>
                  {choice.text}
                </label>
              </div>
            ))}
      ...
      break;
    case "HIDDEN":
      return <input type="hidden" name={field.field.id} id={field.field.id} />;
      break;
    default:
      break;
  }
