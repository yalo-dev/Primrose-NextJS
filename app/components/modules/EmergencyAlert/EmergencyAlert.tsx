...
const EmergencyAlert = () => {
  ...
  const isAlertExpired = (expirationDate) => {
    ...
    const isAfterMidnight =
      currentDate.getTime() > expirationDateTime.getTime() &&
      currentDate.getTime() > expirationDateTime.setHours(23, 59, 59, 999);

    return isAfterMidnight;
  };

  const displayEmergencyAlert =
    data?.school?.schoolAdminSettings?.displayEmergencyAlert;
  ...
  return (
    <div className="alert-module container">
      ...
      {!loading && !error && data && data.school && emergencyMessages && (
        <div
          className={`emergency-alert${dismissedAlerts.every(Boolean) ? "" : " margin-top"}`}
        >
          {emergencyMessages.map(
            (emergencyMessage: EmergencyMessage, index: number) =>
              !dismissedAlerts[index] &&
              !isAlertExpired(emergencyMessage.expirationDate) && (
                        ...
                        {emergencyMessage.message && (
                          <div
                            ...
                            dangerouslySetInnerHTML={{
                              __html: emergencyMessage.message,
                            }}
                          />
                        )}
                      </div>
                      ...
                      >
                        ...
                        >
                          ...
                          <rect
                            ...
                            width="1.28571"
                            height="9"
                            transform="rotate(45 13.2266 6.86296)"
                            fill="#5E6738"
                          />
                          <rect
                            x="6.86328"
                            ...
                            width="1.28571"
                            height="9"
                            transform="rotate(-45 6.86328 7.77239)"
                            fill="#5E6738"
                          />
                        </svg>
                      </button>
