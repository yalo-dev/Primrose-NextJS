...
const FindASchoolMap: React.FC<FindASchoolMapProps> = (props) => {
  ...
  useEffect(() => {
    if (mapRef.current && nearInputRef.current?.value && !searched)
      onEnterKeyPressed();
  }, [mapRef.current]);
  useEffect(() => {
    if (!schools?.length) {
    ...
    function setMapScrollerHeight() {
      ...
      if (scroller && scroller.offsetHeight) {
        ...
        scroller.style.height =
          map.offsetHeight -
          (scroller.offsetTop + scrollerParent.offsetTop) +
          "px";
      }
    }
  }, []);
  ...
  const renderRoute = () => {
    ...
    const startLocationNotFormatted =
      !start ||
      (typeof start !== "string" &&
        (typeof start !== "object" || !start.lat || !start.lng));
    const destinationNotDefined =
      !destination ||
    ...
    if (
      googleMapsNotLoaded ||
      startLocationNotFormatted ||
      destinationNotDefined
    )
      return;
    ...
    directionsService.route(route, (result, status) => {
      if (status === window.google.maps.DirectionsStatus.OK) {
        ...
        let routeBounds =
          directionsRendererRef.current.getDirections().routes[0].bounds;
        let routeCenter = {
          lng: routeBounds.getCenter().lng(),
          lat: routeBounds.getCenter().lat(),
        };
  ...
  function onPlaceSelected(place, type = "text") {
    if (place && place.geometry && place.geometry.location) {
      ...
      if (route != null) {
        let routeCenter = {
          lat: route.routes[0].bounds.getCenter().lat,
          lng: route.routes[0].bounds.getCenter().lng,
        };
        setMapCenter(routeCenter);
      } else {
  ...
  return (
    <div
      id="map"
      className={"find-a-school-container " + (title ? "title" : "")}
    >
      {title && (
                    ...
                    <ul className="options">
                      {school.preopening && (
                        <li className="text-capitalize">
                          Opening {school.openingInSeason}{" "}
                          {school.openingInYear}
                        </li>
                      )}
              ...
              <a href={`${school.uri}`}>
                ...
                >
                  ...
                  <ul className="options">
                    {school.preopening && (
                      <li className="text-capitalize">
                        Opening {school.openingInSeason} {school.openingInYear}
                      </li>
                    )}
                    {school.corporateChildcare && <li>Corporate Child Care</li>}
                  </ul>
                  <div className="button-wrap d-flex">
                    <Button
                      className="button primary"
