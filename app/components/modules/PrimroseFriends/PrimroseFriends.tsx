...
const PrimroseFriends: React.FC<PrimroseFriends> = ({
  tabs,
  customizations,
}) => {
  ...
  useEffect(() => {
    const handleScrollPF = () => {
      if (containerRefPF.current) {
        const containerTopPF =
          containerRefPF.current.getBoundingClientRect().top;
        ...
        const lastContentOffsetPF = document
          .querySelector(".primrose-friends .desktop-content:last-child")
          ?.getBoundingClientRect().bottom;

        if (
          containerTopPF <= 120 &&
          lastContentOffsetPF &&
          lastContentOffsetPF > 0
        ) {
          setIsStickyPF(true);
        } else {
          setIsStickyPF(false);
  ...
  const handleLabelClickPF = (targetId: string) => {
    ...
    if (window.innerWidth > 992) {
      ...
      if (targetElementPF) {
        ...
        const absoluteTargetTopPF =
          targetElementPF.getBoundingClientRect().top + offsetPF;
        ...
        if (containerRefPF.current) {
          const innerDivPF = containerRefPF.current.querySelector(
            ".primrose-friends .inner",
          );
          if (innerDivPF) {
      ...
      if (
        expandedTabPF !== null &&
        `pf-content-${expandedTabPF}` === targetId
      ) {
  ...
  return (
    <div
      className={`container ${isStickyPF ? "sticky" : ""}`}
      ref={containerRefPF}
    >
      <Customizations
        ...
        <div className="primrose-friends">
            ...
            {tabs.map((tab, index) => (
              <div className="label" key={index}>
                ...
                <div className="d-lg-none">
                  {expandedTabPF === index && (
                    <animated.div
                      style={slideAnimationPropsPF}
                      className={`tab-content ${tab.friendColor || ""}`}
                      id={`pf-content-${index}`}
                    >
                      <div className="wrap p-3">
                        ...
                        <div className="content-wrapper">
                          ...
                          <div className="bio">
                            {tab.content.bio && <p>{tab.content.bio}</p>}
                          </div>
                          {tab.content.learnMore &&
                            tab.content.learnMore.url && (
                              <Link
                                href={tab.content.learnMore.url}
                                className="link learn-more b2"
                                target={tab.content.learnMore.target || "_self"}
                              >
                      ...
                      <div className="video-wrapper flex-column flex-lg-row p-3">
                        {tab.content.watchNow && (
                          <div className="wrap pe-4">
                            ...
                            {tab.content.watchNow && (
                              <span className="b3">{tab.content.watchNow}</span>
                            )}
                          </div>
                        )}
                        {tab.content?.videoUrl?.url && (
                          <div className="video">
                            {playingVideoPF !== index && (
                              <PlayButton
                                onPlay={() => toggleVideoPlayback(index)}
                              />
                            )}{" "}
                            {/* Only show play button if this video is not playing */}
                            <video
                              width="320"
                              height="240"
          ...
          <div className="desktop-content d-none d-lg-block col-lg-7 offset-lg-1">
            {tabs.map((tab, index) => (
              <div
                id={`pf-content-${index}`}
                className={`tab-content d-flex ${tab.friendColor || ""}`}
                key={index}
                ref={(el) => (contentRefsPF.current[index] = el)}
              >
                <div className="wrap">
                  {tab.content?.image?.sourceUrl && (
                    <div className="image-wrapper">
                  ...
                  <div className="content-wrapper">
                    {tab.content.name && (
                      <Heading level="h3" color={tab.content.nameColor}>
                        <div
                          dangerouslySetInnerHTML={{ __html: tab.content.name }}
                        />
                      </Heading>
                    )}
                    {tab.content.characterTrait && (
                ...
                {tab.content?.videoUrl?.url && (
                  <div className="video-wrapper pt-3 flex-column flex-lg-row">
                    <div className="wrap d-flex flex-column  align-items-end watch-wrapper">
                      <div className="h5 text-end">
                        <b>Watch Now:&nbsp;</b>
                      </div>
                      {tab.content.watchNow && (
                        <Heading level="h5" className="b3">
                          {tab.content.watchNow}
                        </Heading>
                      )}
                    </div>
                    ...
                    <div className="video">
                      {playingVideoPF !== index && (
                        <PlayButton onPlay={() => toggleVideoPlayback(index)} />
                      )}{" "}
                      {/* Only show play button if this video is not playing */}
                      <video
                        width="320"
