...
const TwoColumnsImageAndText: React.FC<TwoColumnsGreenBackground> = ({
  leftColumn,
  rightColumn,
  customizations,
}) => {
    ...
    >
      <div className="bg-color">
        <div className="container">
          <div
            className={`two-columns-green-background ${displayBackgroundShape ? "with-background" : ""}`}
          >
            {(mobileImageUrl || desktopImageUrl) && (
              <div className="left-column col-12 col-lg-5 offset-lg-1">
                <div className="d-block d-lg-none mb-4">
                  {rightColumn?.heading && (
                    <Heading level="h2">{rightColumn.heading}</Heading>
                  )}
                </div>
                {mobileImageUrl && (
                  <img
            ...
            <div className="right-column col-12 c col-lg-5 offset-lg-1">
              ...
              {rightColumn?.subheading && (
                <Subheading level="h5">{rightColumn.subheading}</Subheading>
              )}
              {rightColumn?.blurb && (
                <div
                  className="blurb"
