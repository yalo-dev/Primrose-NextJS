...
> = ({ options, onSelect, placeholder = "Select" }) => {
  ...
  return (
    <div
      className={`custom-multi-select-dropdown multi-select-dropdown custom-select ${isOpen ? "active" : ""}`}
      ref={dropdownRef}
    >
      ...
      <div className="options" ref={optionsRef}>
        {options.map((opt) => (
          <div
            key={opt.value}
            className={`option ${opt.className ? opt.className : ""} ${selected.includes(opt.value) ? "active" : ""}`}
          >
            <input
              type="checkbox"
              id={opt.value}
