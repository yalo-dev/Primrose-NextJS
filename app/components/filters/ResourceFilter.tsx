  ...
  useEffect(() => {
    ...
    if (selectedTopic.length > 0) {
      newResources = newResources.filter((resource) =>
        resource.resourceTags.nodes.some((tag) =>
          selectedTopic.includes(tag.slug.toLowerCase()),
        ),
      );
    }
    ...
    if (selectedAge.length > 0) {
      newResources = newResources.filter((resource) =>
        resource.resourceTags.nodes.some((tag) =>
          selectedAge.includes(tag.slug.toLowerCase()),
        ),
      );
    }
    ...
    if (searchTerm) {
      ...
      newResources = newResources.filter((resource) => {
        ...
        const tagMatch = resource.resourceTags.nodes.some((tag) =>
          tag.name.toLowerCase().includes(searchLower),
        );
        const typeMatch = resource.resourceTypes.nodes.some((type) =>
    ...
    <div className="search-and-filter">
      ...
      <div className="filters">
        {category !== "for-educators" &&
          filterTerms?.resourceTags?.nodes?.find((tag) => tag.slug === "ages")
            ?.children?.nodes?.length > 0 && (
            <MultiSelectDropdown
              options={
                filterTerms?.resourceTags?.nodes
                  ?.find((tag) => tag.slug === "ages")
        ...
        {filterTerms?.resourceTags?.nodes?.find((tag) => tag.slug === "topics")
          ?.children?.nodes?.length > 0 && (
          <MultiSelectDropdown
            options={
              filterTerms?.resourceTags?.nodes
                ?.find((tag) => tag.slug === "topics")
                ?.children?.nodes?.map((child) => ({
                  label: child.name,
                  value: child.slug,
                })) || []
